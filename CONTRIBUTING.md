...
1. `pip install Backend/api/requirement.txt`
2. Create a `.env` file in the `Backend/` directory (this file is gitignored so your settings stay local).
3. Inside `.env`, add this exact line: `DATABASE_URL=postgresql://postgres:12345678@database-1.c36wyoowwijy.us-east-2.rds.amazonaws.com:5432/postgres`
4. (Deprecated) Within 1 second, your local `dev.db` is populated and ready for frontend and backend component development!
...
_The Swagger UI is your best friend. It allows you to manually execute and test your APIs (like logging in or generating mock data) by simply clicking buttons in the browser._
...
_Because the server is running with `--reload`, your new API will instantly appear on your Swagger UI page._
...
2. Run your tests in the terminal: `pytest Backend/tests/`
   _If your tests fail, do not push your code._
...
4. Inside `.env`, add this line to connect to your local backend:
   ...
   ```
   _(Note: Never hardcode `http://localhost:8000` in your React components. Always use `import.meta.env.VITE_API_BASE_URL`.)_
